...
class Categories:
    ...
    #     def recursive_find_subcategory(categories, category): 
    #         """Find a category and return the category and its subcategories flattened"""
    #         if type(categories) is list:
    #             for i, cat in enumerate(categories):
    #                 if type(cat) is list:
    #                     result = recursive_find_subcategory(cat, category)
    #                     if result:
    #                         return result
    #                 elif cat == category:
    #                     return self._flatten(([cat] + categories[i+1]) if i+1 < len(categories) and type(categories[i+1]) is list else [cat])
    #         return []
    #     return recursive_find_subcategory(self._categories, category)
    #     Rewrite the find_subcategories method of the Categories class using a generator.
    # Define an inner function find_subcategories_gen, which is a recursive generator, in the find_subcategories method.
    # For find_subcategories itself, simply return a list generated by find_subcategories_gen.
    # Remove the _flatten method, which is no longer needed.
